...
# Set optimizations for PyTorch 2.2.2
...
def main():
    ...
    # Initialize symbols based on configuration and validate
    try:
        ...
        # Set n_symbols in hps for model initialization
        hps.data.n_symbols = len(symbols)
        
        if len(symbols) == 0:
            raise ValueError("No symbols loaded - check text cleaners configuration")
...
def run(rank, n_gpus, hps):
    ...
    try:
        ...
        global_step = 0
        if rank == 0:
            logger.info(f"Starting training from scratch: {e}")
