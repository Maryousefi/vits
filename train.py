...
def main():
    """Single-node multi-GPU training (assumes NCCL available)."""
    assert torch.cuda.is_available(), "CPU training is not allowed."

    n_gpus = torch.cuda.device_count()
    os.environ["MASTER_ADDR"] = "localhost"
    # valid port (must be < 65536). 29500 is common default for torch.distributed
    os.environ["MASTER_PORT"] = "29500"

    hps = utils.get_hparams()

    # sanity checks for filelists
    tf = getattr(hps.data, "training_files", None)
    vf = getattr(hps.data, "validation_files", None)
...
def run(rank, n_gpus, hps):
    ...
    train_loader = DataLoader(
        train_dataset, num_workers=8, shuffle=False, pin_memory=True, collate_fn=collate_fn, batch_sampler=train_sampler
    )

    if rank == 0:
        eval_dataset = TextAudioLoader(hps.data.validation_files, hps.data)
        eval_loader = DataLoader(
            eval_dataset, num_workers=8, shuffle=False, batch_size=hps.train.batch_size, pin_memory=True, drop_last=False, collate_fn=collate_fn
        )
    else:
        eval_loader = None

    # Determine symbol set (supports the utils auto-loaded variant as well)
    if hasattr(hps.data, "symbols"):
        symbols = hps.data.symbols
    else:
        symbols = get_symbols(hps.data.text_cleaners)
    n_symbols = len(symbols)

    net_g = SynthesizerTrn(n_symbols, hps.data.filter_length // 2 + 1, hps.train.segment_size // hps.data.hop_length, **hps.model).cuda(rank)
