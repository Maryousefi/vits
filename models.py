...
# FIXED: Import from the package level, not from core directly
...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, x_lengths):
        ...
        m, logs = torch.split(stats, self.proj.out_channels // 2, dim=1)
...
class ResidualCouplingBlock(nn.Module):
    def __init__(self, channels, hidden_channels, kernel_size, dilation_rate, n_layers, n_flows=4, gin_channels=0):
        super().__init__()
        self.channels = channels  # ADDED: Store channels as attribute
        self.flows = nn.ModuleList()
        for i in range(n_flows):
...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid=None):
        ...
        g = self.emb_g(sid).unsqueeze(-1) if hasattr(self, "emb_g") else None
        ...
        with torch.no_grad():
            ...
            # FIXED: Use the imported maximum_path function
        ...
        # FIXED: Use z.size(1) instead of self.flow.channels
        z_slice, ids_slice = commons.rand_slice_segments(z, y_lengths, z.size(1))
